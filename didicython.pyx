...
cdef class PurePython:
    cdef str input_dir, output_dir, locationpath
    cdef dict images, results
    ...
    cpdef np.ndarray gaussian_filter(self, np.ndarray image):
        cdef np.ndarray kernel = np.array([[1, 2, 1], [2, 4, 2], [1, 2, 1]], dtype=np.float32) / 16
        return cv2.filter2D(image, -1, kernel)
    
    cpdef np.ndarray sobel_filter(self, np.ndarray image):
        cdef np.ndarray Gx = cv2.Sobel(image, cv2.CV_64F, 1, 0, ksize=3)
        cdef np.ndarray Gy = cv2.Sobel(image, cv2.CV_64F, 0, 1, ksize=3)
        return np.sqrt(Gx**2 + Gy**2)
    
    cpdef np.ndarray median_filter(self, np.ndarray image, int kernel_size=3):
